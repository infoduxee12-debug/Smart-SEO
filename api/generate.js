/**
 * Vercel Serverless Function for Social Script Engine API
 * Supports ESM format since the project uses "type": "module"
 */
...
export default async function handler(req, res) {
  ...
  // Setup the strict compliance schema
  const schema = {
    type: Type.OBJECT,
    properties: {
...
Each hashtag in the arrays MUST start with the '#' prefix (e.g., "#MindsetMatters"). Do not include any other words in the lists. Ensure strict compliance to the counts: exactly 5 for Facebook, exactly 15 for Instagram, and exactly 15 for TikTok.`;
  ...
  // Check if API key is provided, if not, jump directly to local fallback to facilitate zero-config Vercel deploy
  if (!apiKey) {
    console.warn("GEMINI_API_KEY environment variable is not set. Executing the offline local dynamic trend generator.");
    return executeLocalFallback(script, res);
  }
  ...
  try {
    ...
    try {
      ...
      console.warn("Vercel Serverless: Grounded Search rate-limited. Falling back to non-grounded high-performance prediction...", groundingError);
      usedFallback = true;
...
Each hashtag in the arrays MUST start with the '#' prefix (e.g., "#MindsetMatters"). Do not include any other words in the lists. Ensure strict compliance to the counts: exactly 5 for Facebook, exactly 15 for Instagram, and exactly 15 for TikTok.`;
    ...
    // Get Web Grounding Citations metadata
    ...
    console.error("Vercel Serverless Error, falling back to local processing:", error);
    return executeLocalFallback(script, res);
  }
}

// Completely self-contained high-fidelity local trend generator
function executeLocalFallback(script, res) {
  ...
  // Custom headline snippet formulation
